<template>
	<view>
		<view class="myaddress-lis" v-for="(item,index) in mainData">
			<view class="name" @click="choose(index)">{{item.name}}<view class="numb">{{item.phone}}</view>
			</view>
			...
			<view class="seltBox">
				<view class="L" :data-id="item.id" @click="updateAddress($event.currentTarget.dataset.id)">
					<image class="icon"  alt=""
					:src="item.isdefault==1?'../../static/images/pay-icon5.png':'../../static/images/pay-icon6.png'"
					></image>
					默认地址
				</view>
				<view class="R">
					<view class="child" :data-id="item.id" @click="Router.navigateTo({route:{path:'/pages/myAddressAdd/myAddressAdd?id='+$event.currentTarget.dataset.id}})">
						<image src="../../static/images/adderss-icon1.png" mode=""></image>编辑
					</view>
					<view class="child" :data-id="item.id" @click="deleteAddress($event.currentTarget.dataset.id)">
						<image src="../../static/images/address-icon2.png" mode=""></image>删除
					</view>
				</view>
		...
		<view class="submitbtn" style="margin-top: 200rpx;">
			<button class="hei" type="button" @click="Router.navigateTo({route:{path:'/pages/myAddressAdd/myAddressAdd'}})">添加地址</button>
		</view>
	</view>
</template>
